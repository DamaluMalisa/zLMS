...
@Component({
  ...
  template: `
    ...
    <ng-container *ngIf="showContent; else loadingSkeleton">
      <div class="text-warning px-[25px] pb-[50px] pt-[5px]" *ngIf="assignments.length === 0 && !createButtonClicked && authService.validateRoles(['ROLE_TEACHER'], 'any')">
      <nz-empty
    ...
    >
      ...
      <ng-template #footerTpl>
        <button nz-button class="bg-primary hover:bg-primary-hbr border-solid border-1 border-primary text-white text-[14px] font-semibold leading-[22px] inline-flex items-center justify-center rounded-[4px] px-[20px] h-[44px]" (click)="createAssignment()">Create Assignment</button>
      </ng-template>
    </nz-empty>
      </div>
              ...
              <!-- <div nz-row nzGutter="25" class="mt-[4px]">
                <div nz-col nzXs="24" nzMd="12" class="mb-[25px]">
                  <div class="flex items-center flex-wrap gap-[20px]  max-lg:justify-center">
                    ...
                    <div class="3xl:w-[280px] border-1 border-black max-ssm:w-full [&>div>div.ant-select-selector]:border-0">
                ...
                <div nz-col nzXs="24" nzMd="12" class="mb-[25px]" *ngIf="authService.validateRoles(['ROLE_TEACHER'], 'any')">
                  ...
                  <div class="flex items-center justify-end max-md:justify-center">
                    <button class="flex items-center px-[25px] text-sm text-white font-semibold bg-primary border-primary h-11 gap-[6px]"
                            nz-button (click)="createAssignment()">
                      <i nz-icon nzType="plus"></i>
                      <span class="m-0">Assignment</span>
                    </button>
                  </div>
          ...
          <div class=" py-[30px] px-[25px]  border-b border-regular dark:border-white/10">
            <div>
              <div class="px-[80px] max-sm:px-0">
                <nz-form-label class="flex items-center font-medium dark:text-white/60" [nzSm]="6" [nzXs]="24" nzFor="Assignment Content"
                               nzRequired>Assignment Content
                </nz-form-label>
                <div
...
export class AssignmentsComponent implements OnInit {
  ...
  createAssignment() {
    this.isAssignmentListOpen = false;
    this.createButtonClicked = true;
  }

  cancelAssignment() {
    this.isAssignmentListOpen = true;
  ...
  onSubmit(publish: boolean): void {
    ...
    const assignment: Assignment = {
      ...
      assignmentContent: this.editorContent,
      timestamp: new Date(),
    ...
    this.assignmentService.create(assignment).subscribe(
      (response) => {
        ...
        this.editorContent = '';
        this.router.navigate(['/pages/subject', subjectId, 'assignments']).then(() => {
  ...
  grader(assignment: Assignment) {
    this.router.navigate(
      ['/changelog/grader', assignment.id],
    ).catch(error => {
      console.error('Navigation error:', error);
    });
  }
  ...
  createMessage(type: string): void {
    this.message.create(type, `${type} Assignment created successfully:`);
  }
